...
### What is the Problem Solving and Decision Modeling Skill?
`59-problem-solving-decision-modeling-skill` (owned by CEO-Agent, triggered by `/solve` or plain English) is a reasoning engine, not a PRD generator. It frames an ambiguous problem, models the current state and its causal chain *before* proposing a fix, builds a metric tree, writes falsifiable hypotheses, compares multiple solution options, and selects the right formula from a 29-formula reusable library (`skills/59-problem-solving-decision-modeling-skill/FORMULA_LIBRARY.md`) — unit economics, funnels, capacity, reliability, experimentation, prioritization, TCO, and more. Unlike the brand or product Skills, it's cross-cutting: it activates *alongside* whichever domain Agent a decision touches (CIO-Agent for a robotics call, CFO-Agent for a pricing call) rather than owning a domain of its own. See `workflows/11-problem-solving-decision-workflow/WORKFLOW.md` and `examples/decision-modeling-example.md`.
