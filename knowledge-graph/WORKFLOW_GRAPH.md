...
## Nodes
`01-new-product-workflow`, `02-saas-product-workflow`, `03-hardware-product-workflow`, `04-ai-product-workflow`, `05-robotics-product-workflow`, `06-fundraising-workflow`, `07-go-to-market-workflow`, `08-company-building-workflow`, `09-hiring-workflow`, `10-strategic-planning-workflow`, `11-problem-solving-decision-workflow`.
...
- **CBO-Agent runs inside 9 of the 10 product/company workflows by default** — naming/positioning in 01, in-product voice in 02, CMF/packaging in 03, AI persona in 04, naming/CMF/HRI cues in 05, narrative arc in 06, positioning/messaging first in 07, employer brand in 08, brand/positioning implications in 10. **09 (Hiring) is the deliberate exception** — CBO-Agent is conditional there, brought in only if a job posting is externally visible, consistent with 09's stated design as the most agent-light workflow in the system
- **11 (Problem Solving and Decision) is a different kind of node** — it isn't a request-shape workflow like 01-10, it's the reasoning workflow those ten call into at any decision, prioritization, or validation gate (each of `01`-`10`'s "Validation" sections references it). It can also run standalone, triggered directly by a decision-shaped request rather than a product-shape one
...
| 11 Problem Solving and Decision | Independent — triggered directly by a decision-shaped request, or invoked by any of 01-10 at their Validation gate; not sequenced after another workflow's output the way 06/07 are |
...
A standalone decision request ("should we self-host our model's inference or stay on a managed API?") runs `11-problem-solving-decision-workflow` directly — no product workflow needs to be running first. If the same question surfaces mid-way through `04-ai-product-workflow` instead, it's handled at that workflow's Validation gate rather than switching workflows.
